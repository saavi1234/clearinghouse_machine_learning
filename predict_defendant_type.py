#!/usr/bin/env python3
"""Predict defendant types from case JSON input.

This script loads the trained defendant type artifacts from:
  models/defendant_type/
...
DEFAULT_MODEL_DIR = SCRIPT_DIR / "models" / "defendant_type"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Predict defendant type labels from case JSON input."
    )
    parser.add_argument(
        "--input",
        "-i",
    ...
    parser.add_argument(
        ...
        help="Directory containing defendant_model.pkl, tfidf_vectorizer.pkl, and mlb.pkl.",
    )
    parser.add_argument(
        "--pretty",
        action="store_true",
...
def case_to_text(case: Dict[str, Any], case_idx: int) -> str:
    ...
    # prioritize docs whose title contains key legal terms.
    ...
    for doc in docs:
        ...
        combined = f"{title.lower()} {text}".strip()
        if not combined:
            continue

        if any(term in title.lower() for term in HIGH_PRIORITY_TERMS):
            priority_texts.append(combined)
        else:
            secondary_texts.append(combined)

    return " ".join(priority_texts + secondary_texts)
...
def load_artifacts(model_dir: Path):
    model_path = model_dir / "defendant_model.pkl"

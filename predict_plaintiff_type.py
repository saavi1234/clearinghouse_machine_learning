#!/usr/bin/env python3
"""Predict plaintiff types from case JSON input.

This script loads the trained plaintiff type artifacts from:
  models/plaintiff_type/
...
DEFAULT_MODEL_DIR = SCRIPT_DIR / "models" / "plaintiff_type"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Predict plaintiff type labels from case JSON input."
    )
    parser.add_argument(
        "--input",
        "-i",
    ...
    parser.add_argument(
        ...
        help="Directory containing plaintiff_model.pkl, tfidf_vectorizer.pkl, and mlb.pkl.",
    )
    parser.add_argument(
        "--pretty",
        action="store_true",
...
def case_to_text(case: Dict[str, Any], case_idx: int) -> str:
    ...
    # "{title} {text}" per document, joined by blank lines.
...
def load_artifacts(model_dir: Path):
    model_path = model_dir / "plaintiff_model.pkl"
